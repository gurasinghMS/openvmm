...
export function LogViewer(): React.JSX.Element {
  ...
  const columns = useMemo(
    () => createColumns(setScreenshot, pinnedIndices),
    [pinnedIndices],
  );

  // Conditional AND wildcard search, but always include pinned rows
  const filteredLogs = useMemo(
    () => filterLog(logEntries, searchFilter, pinnedIndices),
    [logEntries, searchFilter, pinnedIndices],
  );
      ...
      >
        <VirtualizedTable<LogEntry>
          ...
          getRowClassName={(row) => {
            ...
            const isPinned = pinnedIndices.has(row.original.index);
            const severityClass = `severity-${row.original.severity}`;
            return `${severityClass} ${isSelected ? "selected" : ""} ${isPinned ? "pinned" : ""}`;
          }}
          overscan={100}
          ...
          onRowDoubleClick={(row) => {
            // Clear any text selection from double-click
            window.getSelection()?.removeAllRanges();
            // Toggle pin state for this row
 ...
 * @param pinnedIndices - Set of log indices that are pinned (always included regardless of filter)
...
function filterLog(
  ...
  // Filter logs - check if each log matches all tokens, but always include pinned rows
  return logs.filter((log) => {
    // Always include pinned rows regardless of search criteria
    if (pinnedIndices.has(log.index)) return true;
